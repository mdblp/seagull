 ...
 */
module.exports = function (crudHandler, userApiClient, gatekeeperClient, metrics, serverConfig) {
        ...
        gatekeeperClient.usersInGroup(targetUserId, async function(error, trusteeUserPermissions) {
          ...
          try {
            ...
            if (userProfiles.length !== userIds.length) {
              const profileIds = userProfiles.map(u => u.userid);
              const missingProfiles = userIds.filter(x => !profileIds.includes(x));
              log.error('Missing user profiles:', missingProfiles);
            }
          } catch (error) {
            log.error(`GET /users/${targetUserId}/users 500`, error);
            res.send(500);
            return next(false);
          }
