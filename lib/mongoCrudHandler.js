...
const log = createLogger('mongoCrudHandler.js');
...
module.exports = function init(config) {
  ...
  const _status = { running: false, deps: { up: [], down: [] } };

  let initialConnection = false;
  const db = mongojs(config.mongoConnectionString, ['seagull']);
  ...
  // Running a ping command in order to trigger connect or error event on startup
  ...
  function getDocs(userIds, done) {
    const out= {};
    db.seagull.find({userId: {$in: userIds}}).forEach(
      function (err, doc) {
        if(err) {
          done({statusCode: 404, message: 'not found', detail: err}, null);
        }
        if (!doc) {
            return done(null, {statusCode: 200, message: 'found', detail: out});
        }
        const decrypted = _decrypt_value(doc.value);
        if (!decrypted) {
          done({statusCode: 401, message: 'not authorized', detail: null}, null);
        }
        out[doc.userId] = decrypted
      }
    );
  }
