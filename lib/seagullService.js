...
function createServer(serverConfig, crudHandler, userApiClient, opaClient, teamsClient) {
  ...
  var requireCustodian = function (req, res, next) {
    if(opaClient.selfAuthorized(req,[req.params.userid])) {
      return next();
    }
  ...
  var requireServer = function (req, res, next) {
    if(opaClient.serverAuthorized(req)) {
      return next();
    }
    log.error('do not have the required information to check permissions');
    res.send(401, 'Unauthorized');
  ...
  var requireMembership = async function (req, res, next) {
    log.debug('checking permission [%j]', req.params);
    const auth = await opaClient.isAuthorized(req,[req.params.userid]);
    if (!auth) {
      log.error('do not have the required information to check permissions');
      res.send(401, 'Unauthorized');
      return next(false);
 ...
 */
  ...
  var seagullApi = require('./routes/seagullApi')(crudHandler, userApiClient, teamsClient, serverConfig);
...
module.exports = function seagullService(envConfig, crudHandler, userApiClient, opaClient, teamsClient) {
  var server = null;
